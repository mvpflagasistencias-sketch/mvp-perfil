...
const PerfilJugador = ({ jugadorId, onLogout }) => {
  ...
  // 📢 NUEVO ESTADO: Guarda la promoción seleccionada
  ...
  useEffect(() => {
    const fetchPerfilYEquipos = async () => {
      ...
      try {
        ...
        // Sincronizamos también el historial directamente en el perfil si tu vista lo lee de ahí
        if (resAsistencias.data) {
          ...
          setDatosEquipo(prev => ({
            ...
            totalesPartidos: resAsistencias.data.totalesPartidos || 16,
  ...
  return (
    <div style={{ 
      ...
      >
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            {fotoBase64 ? (
              <img src={fotoBase64} alt="Perfil Real" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ) : (
        ...
        <div style={{ backgroundColor: '#0f172a', padding: '24px', borderRadius: '20px', border: '1px solid #30363d', width: '100%', boxSizing: 'border-box', marginTop: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ backgroundColor: 'white', padding: '12px', borderRadius: '16px', display: 'inline-block' }}>
              <QRCodeSVG 
                ...
                level={"H"} 
                includeMargin={true}
                imageSettings={{ src: logoMvp, height: 60, width: 60, align: 'center', excavate: true }}
              />
            </div>

            <p style={{ marginTop: '16px', marginBottom: '4px', fontSize: '9px', color: '#94a3b8', textAlign: 'center', lineHeight: '1.4', padding: '0 10px' }}>
            ...
            <button 
              ...
              style={{ width: '100%', backgroundColor: '#22c55e', color: 'white', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '900', textTransform: 'uppercase', fontSize: '12px', cursor: 'pointer' }}
            >
        ...
        <div style={{ marginBottom: '12px', width: '100%', marginTop: '20px' }}>
      ...
      <div style={{
          ...
          {tabActiva === 'promos' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {datosEquipo.promociones.length === 0 ? (
                <div style={{ fontSize: '11px', color: '#64748b', textAlign: 'center', padding: '20px', fontStyle: 'italic', fontWeight: '700' }}>
          ...
          {tabActiva === 'asistencias' && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: '20px', width: '100%', boxSizing: 'border-box' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '95px', height: '95px', borderRadius: '50%', border: '6px solid #0f172a', borderTopColor: '#22c55e', marginBottom: '16px' }}>
                <span style={{ fontSize: '24px', fontWeight: '900', color: '#ffffff' }}>{perfil?.asistencias || datosEquipo.asistencias || 0}</span>
              </div>
              ...
              <div style={{ width: '100%', maxWidth: '340px', backgroundColor: '#1e293b', borderRadius: '16px', border: '1px solid #334155', padding: '12px', boxSizing: 'border-box', maxHeight: '220px', overflowY: 'auto', textAlign: 'left' }}>
                {(perfil?.historial || datosEquipo.historial) && (perfil?.historial || datosEquipo.historial).length > 0 ? (
                  (perfil?.historial || datosEquipo.historial).map((asistencia, index) => (
                    <div key={index} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: index < (perfil?.historial || datosEquipo.historial).length - 1 ? '1px solid #334155' : 'none' }}>
                      <div>
        ...
        <div style={{ padding: '20px', backgroundColor: '#0f172a', borderTop: '2px solid #ef4444', display: 'flex', flexDirection: 'column', gap: '10px', boxSizing: 'border-box' }}>
          <button 
            onClick={() => { setModalPerfilAbierto(true); setMenuAbierto(false); }}
            style={{ width: '100%', padding: '12px', backgroundColor: '#2563eb', border: 'none', borderRadius: '10px', color: 'white', fontWeight: '900', textTransform: 'uppercase', fontSize: '11px', cursor: 'pointer' }}
          >
          ...
          <button 
            ...
            style={{ width: '100%', padding: '12px', backgroundColor: 'transparent', border: '1px solid #475569', borderRadius: '10px', color: '#9ca3af', fontWeight: '900', textTransform: 'uppercase', fontSize: '11px', cursor: 'pointer' }}
          >
            🚪 Cerrar Sesión
          </button>
        </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', boxSizing: 'border-box' }}>
          <div style={{ backgroundColor: '#1e293b', padding: '24px', borderRadius: '24px', border: '1px solid #30363d', maxWidth: '420px', width: '100%', color: 'white', boxSizing: 'border-box', overflowY: 'auto', maxHeight: '90vh' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h4 style={{ fontSize: '14px', fontWeight: '900', textTransform: 'uppercase', color: '#60a5fa', margin: 0 }}>Ajustes de Cuenta MVP</h4>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: '10px', width: '100%' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    ...
                    <input 
                      ...
                      style={{ backgroundColor: editandoCampos ? '#1e293b' : '#0f172a', border: '1px solid #30363d', borderRadius: '8px', padding: '10px', color: editandoCampos ? 'white' : '#64748b', fontWeight: '700', outline: 'none', textAlign: 'center', width: '100%', boxSizing: 'border-box' }} 
                    />
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', minWidth: '0' }}>
                    ...
                    <select 
                      ...
                      style={{ backgroundColor: editandoCampos ? '#1e293b' : '#0f172a', border: '1px solid #30363d', borderRadius: '8px', padding: '10px', color: editandoCampos ? 'white' : '#64748b', fontWeight: '700', outline: 'none', width: '100%', boxSizing: 'border-box' }}
                    >
      ...
      {/* 🚀 MODAL: DETALLE INMERSIVO DE PROMOCIÓN */}
      {promoSeleccionada && (
        <div 
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '1px solid #30363d', paddingBottom: '12px' }}>
              <span style={{ fontSize: '12px', fontWeight: '900', color: '#22c55e', textTransform: 'uppercase' }}>📢 Beneficio Activado</span>
            ...
            <button onClick={() => setPromoSeleccionada(null)} style={{ width: '100%', padding: '12px', backgroundColor: '#22c55e', border: 'none', borderRadius: '12px', color: 'white', fontWeight: '900', textTransform: 'uppercase', fontSize: '11px', cursor: 'pointer' }}>
              Entendido
            </button>
          </div>
        </div>
