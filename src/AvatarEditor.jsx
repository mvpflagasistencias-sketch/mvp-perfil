...
// 🚀 Nombres reales de los vectores internos del paquete Adventurer para ropa y cabello
const OPCIONES = {
...
const AvatarEditor = ({ jugadorId, configInicial, onGuardarExito }) => {
  ...
  const obtenerDataUriAvatar = () => {
    try {
      ...
      const avatar = createAvatar(estiloAvatar, {
        size: 100,
        // 🚀 PROPIEDADES CLAVE: Desactivamos el recorte de la cara para que dibuje el torso completo
      ...
      // Conseguimos el SVG base
      let svgString = avatar.toString();

      // 🚀 EL HACK CORRECTO: Modificamos las dimensiones del viewBox para que muestre de la cintura para arriba (0 0 100 135)
      svgString = svgString.replace(/viewBox="[^"]*"/, 'viewBox="0 0 100 135"');

      return `data:image/svg+xml;utf8,${encodeURIComponent(svgString)}`;
    } catch (e) {
      console.error("Error generando avatar nativo:", e);
      return '';
    }
  };
  ...
  return (
    <div style={{ backgroundColor: '#1e293b', padding: '20px', borderRadius: '24px', border: '1px solid #30363d', maxWidth: '360px', margin: '0 auto', color: 'white', textAlign: 'center', boxSizing: 'border-box' }}>
      ...
      {/* Tarjeta contenedora vertical limpia */}
      <div style={{ 
        width: '160px', 
        height: '240px', 
