...
const AvatarEditor = ({
  ...
  useEffect(() => {
    if (configInicial) {
      const hairConfig = Array.isArray(configInicial.hair)
        ? configInicial.hair[0]
        : configInicial.hair;
      const hairColorConfig = Array.isArray(configInicial.hairColor)
        ? configInicial.hairColor[0]
        : configInicial.hairColor;
      const clothingConfig = Array.isArray(configInicial.clothing)
        ? configInicial.clothing[0]
        : configInicial.clothing;
      const clothingColorConfig = Array.isArray(configInicial.clothingColor)
        ? configInicial.clothingColor[0]
        : configInicial.clothingColor;
      const eyebrowsConfig = Array.isArray(configInicial.eyebrows)
        ? configInicial.eyebrows[0]
        : configInicial.eyebrows;
      const skinColorConfig = Array.isArray(configInicial.skinColor)
        ? configInicial.skinColor[0]
        : configInicial.skinColor;

      setIndices({
        cabello:
          OPCIONES.cabello.indexOf(hairConfig) >= 0
            ? OPCIONES.cabello.indexOf(hairConfig)
            : 0,
        colorCabello:
          OPCIONES.colorCabello.indexOf(hairColorConfig) >= 0
            ? OPCIONES.colorCabello.indexOf(hairColorConfig)
            : 0,
        ropa:
          OPCIONES.ropa.indexOf(clothingConfig) >= 0
            ? OPCIONES.ropa.indexOf(clothingConfig)
            : 0,
        colorRopa:
          OPCIONES.colorRopa.indexOf(clothingColorConfig) >= 0
            ? OPCIONES.colorRopa.indexOf(clothingColorConfig)
            : 0,
        expresion:
          OPCIONES.expresion.indexOf(eyebrowsConfig) >= 0
            ? OPCIONES.expresion.indexOf(eyebrowsConfig)
            : 0,
        colorPiel:
          OPCIONES.colorPiel.indexOf(skinColorConfig) >= 0
            ? OPCIONES.colorPiel.indexOf(skinColorConfig)
            : 2,
      });
    }
  }, [configInicial]);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("abrir-editor-avatar", escucharApertura);
  }, []);
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/jugadores/${jugadorId}/avatar`,
        {
          method: "PUT",
      ...
      >
        ...
        >
          ...
          {tipoRopa === 1 && (
            <>
              <path
                d="M 28,65 C 30,52 35,45 40,41"
                fill="none"
                stroke={colorDetalles}
                strokeWidth="4"
                opacity="0.8"
              />
              <path
                d="M 92,65 C 90,52 85,45 80,41"
                fill="none"
                stroke={colorDetalles}
                strokeWidth="4"
                opacity="0.8"
          ...
          {tipoRopa === 2 && (
            <>
              <path
                d="M 23,100 C 24,68 33,52 38,44"
                fill="none"
                stroke={colorSombras}
                strokeWidth="6"
              />
              <path
                d="M 97,100 C 96,68 87,52 82,44"
                fill="none"
                stroke={colorSombras}
                strokeWidth="6"
              />
          ...
          {tipoRopa === 3 && (
            <path
              d="M 27,62 C 40,58 80,58 93,62 L 94,74 C 80,70 40,70 26,74 Z"
              fill={colorDetalles}
              opacity="0.85"
            />
          )}
          ...
          {tipoRopa === 4 && (
            <path
              d="M 45,35 C 48,45 72,45 75,35 C 72,48 48,48 45,35"
              fill={colorDetalles}
            />
          )}
          ...
          {tipoRopa === 5 && (
            <>
              <path
                d="M 42,36 C 44,55 40,75 32,95"
                fill="none"
                stroke={colorSombras}
                strokeWidth="2.5"
              />
              <path
                d="M 78,36 C 76,55 80,75 88,95"
                fill="none"
                stroke={colorSombras}
                strokeWidth="2.5"
              />
    ...
    <>
      ...
      {!soloModal && (
        <div
          ...
          onMouseEnter={(e) =>
            !isDragging && (e.currentTarget.style.transform = "scale(1.05)")
          }
          onMouseLeave={(e) =>
              ...
              {[
                ...
                >
                  ...
                  >
                    <button
                      type="button"
                      onClick={() => cambiarOpcion(item.campo, -1)}
                      style={{
                        background: "none",
                        border: "none",
                        color: "#60a5fa",
